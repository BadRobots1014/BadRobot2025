...
public class ElevatorSubsystem extends SubsystemBase {
  ...
  private int rotationCount;
      ...
      // Throughout the code we use left to get the values as the right is following
      // the left so it should be the same.. I hope
      ...
      public double getElevatorEncoder() {
        if (encoder.getPosition() < lastEncoderPosition &&
          (lastEncoderPosition - encoder.getPosition()) > ElevatorConstants.kMaxThresholdForResetPercent) {
        rotationCount += 1;
    } else if (encoder.getPosition() > lastEncoderPosition &&
        (encoder.getPosition() - lastEncoderPosition) > ElevatorConstants.kMaxThresholdForResetPercent) {
      rotationCount -= 1;
    }

    lastEncoderPosition = encoder.getPosition();
    return encoder.getPosition() + (rotationCount * ElevatorConstants.kMaxEncoderBeforeReset);
  }
